...
def check_database(config):
    ...
    try:
        conn = mysql.connector.connect(**config)
        cursor = conn.cursor()
        cursor.execute("SHOW DATABASES LIKE 'hotel_hr_system'")
        if not cursor.fetchone():
            print("  [失败] 数据库 hotel_hr_system 不存在，尚未初始化")
            print("  提示：运行 python init_database.py 一键初始化")
            cursor.close()
            conn.close()
            return False
        cursor.execute("USE hotel_hr_system")
        cursor.execute("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'hotel_hr_system'")
        ...
        cursor.close()
        conn.close()
        print(f"  [OK] 数据库 hotel_hr_system 已存在")
        print(f"       数据表数量：{table_count}")
        print(f"       员工数量：{employee_count}，登录账号数量：{account_count}")
        if table_count < 12:
...
def main():
    parser = argparse.ArgumentParser(description="酒店人事一体化综合管理系统：环境自检")
    ...
    if py_ok and dep_ok and mysql_ok and db_ok:
        ...
        print("  命令行版：cd python && python app.py")
        print("  Web 版：  cd web_app && python app.py  然后浏览器打开 http://127.0.0.1:5000")
    else:
